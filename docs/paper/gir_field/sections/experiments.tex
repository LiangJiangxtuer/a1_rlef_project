...
\subsection{External field-aware adapter study}

To test whether UEFB-G is only self-referential to our own internal field, we add an external adapter study on frozen P4 baseline outputs. For a black-box prediction $P$, low-light input $L$, and paired reference $H$, the adapter defines $\hat{E}=\log(Y_P+\epsilon)-\log(Y_L+\epsilon)$ and compares it with $E^{gt}=\log(Y_H+\epsilon)-\log(Y_L+\epsilon)$. This is an adapter-derived luminance-gain proxy, not a claim that the external method exposes this exact internal variable. Table~\ref{tab:external_adapter} shows that Retinexformer has strong low-pass adapter-field shape agreement ($S_{corr}=0.9547/0.9893$ on real/synthetic), while Zero-DCE++ has moderate shape agreement but large gauge error. Thus UEFB-G can be used in two modes: explicit internal-field evaluation for methods such as GIR-Field, and adapter-field analysis for black-box enhancers under a clearly stated proxy boundary.

\input{tables/external_adapter_lowpass_table}
