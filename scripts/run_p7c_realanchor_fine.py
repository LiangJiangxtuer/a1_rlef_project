...
def planned_runs() -> list[dict]:
    specs = [
        ('P7C_DHEAD_RA015', 'm4j_eshape_multiscale_dhead_realanchor015', 0.015),
        ('P7C_DHEAD_RA020', 'm4j_eshape_multiscale_dhead_realanchor020', 0.020),
        ('P7C_DHEAD_RA030', 'm4j_eshape_multiscale_dhead_realanchor030', 0.030),
    ]
    return [
        {
            'id': variant_id,
            ...
            'rationale': 'P7c fine-sweeps stronger real-domain head-bias anchors around P7B_DHEAD_RA010; no teacher/distill and no scalar rec_by_dataset.',
        }
...
def make_cfg(run: dict, max_steps: int = 1000, train_crop: int = 128, batch_size: int = 8) -> dict:
    return {
        ...
        'protocol': {
            'stage': 'P7c domain-head real-anchor fine route',
            'variant_id': run['id'],
            'variant_name': run['name'],
            'domain_adapter': run['domain_adapter'],
            'real_anchor_weight': float(run['real_anchor_weight']),
            'notes': 'Fine sweep stronger real-domain head-bias anchors after P7B_DHEAD_RA010 near-miss; keep DHEAD and avoid teacher/distill/rec_by_dataset.',
        },
        'model': run['model'],
        'training': {
...
def write_summary(rows: list[dict]) -> Path:
    ...
    table = ROOT / 'results/tables/p7c_realanchor_fine_summary.csv'
    ...
    (ROOT / 'results/tables/p7c_realanchor_fine_summary.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    return table
...
def main() -> None:
    ...
    runs = planned_runs()
    print(json.dumps({'P7C_START': True, 'runs': [(r['id'], r['name']) for r in runs], 'max_steps': args.max_steps, 'parallel': args.parallel, 'gpu_start': args.gpu_start}, indent=2), flush=True)
    rows: list[dict] = []
    ...
    print(json.dumps({'P7C_DONE': True, 'summary': str(table), 'rows': rows}, indent=2), flush=True)
