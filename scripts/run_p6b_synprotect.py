...
def _loss_with_synthetic_weight(synthetic_weight: float) -> dict:
    loss = dict(BASE_M4J_ES_LOSS)
    # Use explicit per-domain reconstruction weights instead of the scalar rec
    # term so synthetic protection is controlled without teacher distillation.
    loss['rec'] = 0.0
    loss['rec_by_dataset'] = {
        'UEFB': 1.0,
        'LOL-v2-real-train': 1.0,
        'LOL-v2-synthetic-train': float(synthetic_weight),
    }
    return loss
...
def planned_runs() -> list[dict]:
    specs = [
        ('P6B_MS_SYN105', 'm4j_eshape_multiscale_synprotect_rec105', 1.05, 'very light synthetic reconstruction upweighting'),
        ('P6B_MS_SYN110', 'm4j_eshape_multiscale_synprotect_rec110', 1.10, 'light synthetic reconstruction upweighting'),
        ('P6B_MS_SYN125', 'm4j_eshape_multiscale_synprotect_rec125', 1.25, 'modest synthetic reconstruction upweighting'),
        ('P6B_MS_SYN150', 'm4j_eshape_multiscale_synprotect_rec150', 1.50, 'strong synthetic reconstruction upweighting'),
    ]
    return [
        {
            'id': variant_id,
            ...
            'seed': 3407,
            'model': dict(P6_MODEL),
            'loss': _loss_with_synthetic_weight(weight),
            'synthetic_rec_weight': weight,
            'rationale': f'P6 multiscale backbone with {description}; no teacher/output distillation.',
        }
        for variant_id, name, weight, description in specs
    ]
...
def make_cfg(run: dict, max_steps: int = 1000, train_crop: int = 128, batch_size: int = 8) -> dict:
    return {
        ...
        'protocol': {
            'stage': 'P6b synthetic-protection structural route',
            'variant_id': run['id'],
            'variant_name': run['name'],
            'synthetic_rec_weight': float(run['synthetic_rec_weight']),
            'notes': 'Keep P6 multiscale backbone and M4J_ES heads; protect synthetic by upweighting only synthetic paired reconstruction. No teacher/output-level distillation.',
        },
        'model': run['model'],
        'training': {
...
def write_summary(rows: list[dict]) -> Path:
    rows = sorted(rows, key=lambda r: float(r['synthetic_rec_weight']))
    table = ROOT / 'results/tables/p6b_synprotect_summary.csv'
    table.parent.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for row in rows for k in row.keys()})
    preferred = ['run','variant_id','variant_name','seed','backbone','backbone_blocks','synthetic_rec_weight','steps','train_crop','batch_size','gpu','elapsed_sec_train','elapsed_sec_total','uefb_psnr','uefb_ssim','uefb_E_MAE','uefb_E_MAE_aligned','uefb_E_corr','real_psnr','real_ssim','real_E_corr','synthetic_psnr','synthetic_ssim','synthetic_E_corr','run_dir','config','log']
    ...
    (ROOT / 'results/tables/p6b_synprotect_summary.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    return table
...
def main() -> None:
    ...
    runs = planned_runs()
    print(json.dumps({'P6B_START': True, 'runs': [(r['id'], r['name']) for r in runs], 'max_steps': args.max_steps, 'parallel': args.parallel, 'gpu_start': args.gpu_start}, indent=2), flush=True)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=max(1, args.parallel)) as ex:
        ...
        for fut in as_completed(futures):
            row = fut.result()
            rows.append(row)
            print(json.dumps({'DONE_RUN': row['run'], 'synthetic_rec_weight': row.get('synthetic_rec_weight'), 'uefb_psnr': row.get('uefb_psnr'), 'real_psnr': row.get('real_psnr'), 'synthetic_psnr': row.get('synthetic_psnr')}, ensure_ascii=False), flush=True)
    table = write_summary(rows)
    print(json.dumps({'P6B_DONE': True, 'summary': str(table), 'rows': rows}, indent=2), flush=True)
