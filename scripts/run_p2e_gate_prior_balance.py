...
from scripts.run_p2c_uefb_recovery import _e_shape_model, _e_shape_loss  # noqa: E402


def _gate_prior_loss(weight: float) -> dict:
    loss = _e_shape_loss(0.010)
    loss['gate_prior'] = float(weight)
    loss['gate_prior_base'] = 0.35
    loss['gate_prior_dark_gain'] = 0.30
    loss['gate_prior_contrast_gain'] = 0.05
    loss['gate_prior_min'] = 0.25
    loss['gate_prior_max'] = 0.75
...
def planned_runs() -> list[dict]:
    return [
        {
            'id': 'P2E_ESHAPE010_GATEPRI002',
            'name': 'eshape010_image_stat_gate_prior002',
            'seed': 3407,
            'base_variant': 'P2C_DGB_ESHAPE010',
            'intervention': 'image_stat_gate_prior',
            'model': _e_shape_model(),
            'loss': _gate_prior_loss(0.02),
            'rationale': 'P2E: keep e_shape=0.010 and add weak low-image-stat gate prior; no dataset labels, no teacher.',
        },
        {
            'id': 'P2E_ESHAPE010_GATEPRI005',
            'name': 'eshape010_image_stat_gate_prior005',
            'seed': 3407,
            'base_variant': 'P2C_DGB_ESHAPE010',
            'intervention': 'image_stat_gate_prior',
            'model': _e_shape_model(),
            'loss': _gate_prior_loss(0.05),
            'rationale': 'P2E: stronger low-image-stat gate prior to improve dark real routing while preserving lower-shape UEFB/synthetic behavior.',
        },
    ]
...
def _p2c_e010_reference() -> dict:
    path = ROOT / 'results/tables/p2c_uefb_recovery_summary.json'
    if not path.exists():
        return {}
    payload = json.loads(path.read_text(encoding='utf-8'))
    for row in payload.get('runs', []):
        if row.get('variant_id') == 'P2C_DGB_ESHAPE010':
            return row
    return {}


def _p2d_best_reference() -> dict:
    path = ROOT / 'results/tables/p2d_balanced_eshape_real_preserve_summary.json'
    if not path.exists():
        return {}
    payload = json.loads(path.read_text(encoding='utf-8'))
    rows = payload.get('runs', [])
    return max(rows, key=lambda r: r.get('uefb_psnr', float('-inf'))) if rows else {}


def run_one(run: dict, max_steps: int, train_crop: int, batch_size: int, gpu: int, resume: bool = False) -> dict:
    tag = f"p2e_{run['id'].lower()}_{run['name']}"
    ...
    log_path = ROOT / 'logs/p2e_gate_prior_balance' / f'{tag}.log'
    ...
    row = {
        ...
        'gate_prior': float(run['loss'].get('gate_prior', 0.0)),
        'e_shape': float(run['loss'].get('e_shape', 0.0)),
...
def write_summary(rows: list[dict]) -> Path:
    ...
    table = ROOT / 'results/tables/p2e_gate_prior_balance_summary.csv'
    table.parent.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for row in rows for k in row.keys()})
    preferred = ['run','variant_id','variant_name','base_variant','intervention','seed','backbone','backbone_blocks','gauge_mode','route_type','route_floor','e_shape','gate_prior','safe_alpha','steps','train_crop','batch_size','gpu','elapsed_sec_train','elapsed_sec_total','uefb_psnr','uefb_ssim','uefb_E_MAE','uefb_E_MAE_aligned','uefb_E_corr','uefb_identity_drop','uefb_q_ece','real_psnr','real_ssim','real_lee','real_nai','real_E_corr','real_identity_drop','real_q_ece','synthetic_psnr','synthetic_ssim','synthetic_lee','synthetic_nai','synthetic_E_corr','synthetic_identity_drop','synthetic_q_ece','run_dir','config','log']
    ...
    payload = {
        ...
        'p2c_eshape010_reference': _p2c_e010_reference(),
        'p2d_best_uefb_reference': _p2d_best_reference(),
        'promotion': [should_promote_to_multiseed(r, p3c, p7b) for r in rows] if p3c else [],
    }
    (ROOT / 'results/tables/p2e_gate_prior_balance_summary.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    return table
...
def main() -> None:
    ...
    runs = planned_runs()
    print(json.dumps({'P2E_START': True, 'runs': [(r['id'], r['name']) for r in runs], 'max_steps': args.max_steps, 'parallel': args.parallel, 'gpu_start': args.gpu_start}, indent=2), flush=True)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=max(1, args.parallel)) as ex:
        ...
        for fut in as_completed(futures):
            row = fut.result()
            rows.append(row)
            print(json.dumps({'DONE_RUN': row['run'], 'gate_prior': row.get('gate_prior'), 'uefb_psnr': row.get('uefb_psnr'), 'real_psnr': row.get('real_psnr'), 'synthetic_psnr': row.get('synthetic_psnr'), 'uefb_E_corr': row.get('uefb_E_corr')}, ensure_ascii=False), flush=True)
    table = write_summary(rows)
    print(json.dumps({'P2E_DONE': True, 'summary': str(table), 'rows': rows}, indent=2), flush=True)
