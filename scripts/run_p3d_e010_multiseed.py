...
def planned_jobs() -> list[dict]:
    return [
        {'seed': 3407, 'e_shape': 0.10, 'e_shape_kernel': 7},
        {'seed': 2027, 'e_shape': 0.10, 'e_shape_kernel': 7},
        {'seed': 42, 'e_shape': 0.10, 'e_shape_kernel': 7},
    ]


def job_tag(job: dict) -> str:
    return f"p3d_m4j_es_seed{int(job['seed'])}_e{int(round(float(job['e_shape']) * 1000)):04d}"
...
def make_cfg(job: dict, max_steps: int = 1000, train_crop: int = 128, batch_size: int = 8) -> dict:
    loss = dict(BASE_LOSS_E010)
    loss['e_shape'] = float(job['e_shape'])
    loss['e_shape_kernel'] = int(job.get('e_shape_kernel', 7))
    return {
        'seed': int(job['seed']),
        'protocol': {
            'stage': 'P3d M4J_ES e_shape=0.10 multi-seed validation',
            'variant_id': 'M4J_ES_E010',
            'variant_name': 'm4_joint_eshape_e010',
            'seed': int(job['seed']),
            'e_shape': float(job['e_shape']),
            'notes': 'Validate e_shape=0.10 across three seeds before P4 official baselines.',
        },
...
def reuse_run_dir(job: dict) -> Path | None:
    if int(job['seed']) == 3407 and abs(float(job['e_shape']) - 0.10) < 1e-12:
        p = ROOT / 'experiments/p3c_m4j_es_seed3407_e0100'
        if p.exists():
            return p
    return None
...
def run_one(job: dict, max_steps: int, train_crop: int, batch_size: int, gpu: int, resume: bool = False) -> dict:
    ...
    cfg_dir = ROOT / 'configs/p3d_e010_multiseed'
    ...
    log_path = ROOT / 'logs/p3d_e010_multiseed' / f'{tag}.log'
    ...
    if reused is not None:
        log_path.parent.mkdir(parents=True, exist_ok=True)
        with log_path.open('a', encoding='utf-8') as log:
            log.write(f'REUSE: seed=3407 e_shape=0.10 imported from {out}\n')
    ...
    row = {
        ...
        'batch_size': batch_size,
        'gpu': gpu,
        'reused_from_p3c': bool(reused is not None),
...
def aggregate(rows: list[dict]) -> dict:
    ...
    return {m: _mean_std([float(r[m]) for r in rows]) for m in metrics}


def should_enter_p4(aggregate_metrics: dict, synthetic_baseline_mean: float = 17.678, min_real_psnr: float = 19.8) -> bool:
    return (
        aggregate_metrics['real_psnr']['mean'] >= min_real_psnr and
        aggregate_metrics['synthetic_psnr']['mean'] > synthetic_baseline_mean and
        aggregate_metrics['uefb_E_corr']['mean'] > 0 and
        aggregate_metrics['synthetic_E_corr']['mean'] > 0
    )


def write_summary(rows: list[dict]) -> Path:
    rows = sorted(rows, key=lambda r: int(r['seed']))
    table = ROOT / 'results/tables/p3d_e010_multiseed_summary.csv'
    table.parent.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for row in rows for k in row.keys()})
    preferred = ['run','variant_id','variant_name','seed','e_shape','e_shape_kernel','steps','train_crop','batch_size','gpu','reused_from_p3c','elapsed_sec_train_or_reuse','elapsed_sec_total','uefb_psnr','uefb_ssim','uefb_E_MAE','uefb_E_MAE_aligned','uefb_E_corr','uefb_identity_drop','uefb_q_ece','real_psnr','real_ssim','real_lee','real_nai','real_E_corr','synthetic_psnr','synthetic_ssim','synthetic_lee','synthetic_nai','synthetic_E_corr','run_dir','config','log']
    ...
    agg = aggregate(rows)
    (ROOT / 'results/tables/p3d_e010_multiseed_summary.json').write_text(json.dumps(rows, indent=2), encoding='utf-8')
    (ROOT / 'results/tables/p3d_e010_multiseed_aggregate.json').write_text(json.dumps({'aggregate': agg, 'enter_p4': should_enter_p4(agg)}, indent=2), encoding='utf-8')
    return table
...
def main():
    ...
    jobs = planned_jobs()
    print(json.dumps({'P3D_START': True, 'jobs': jobs, 'max_steps': args.max_steps, 'parallel': args.parallel, 'resume': args.resume}, indent=2), flush=True)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=args.parallel) as ex:
        ...
        for fut in as_completed(futures):
            row = fut.result()
            rows.append(row)
            print(json.dumps({'DONE_RUN': row['run'], 'seed': row['seed'], 'reused': row['reused_from_p3c'], 'uefb_E_corr': row.get('uefb_E_corr'), 'real_psnr': row.get('real_psnr'), 'synthetic_psnr': row.get('synthetic_psnr')}, ensure_ascii=False), flush=True)
    table = write_summary(rows)
    agg = aggregate(rows)
    print(json.dumps({'P3D_DONE': True, 'summary': str(table), 'aggregate': agg, 'enter_p4': should_enter_p4(agg), 'rows': rows}, indent=2), flush=True)
