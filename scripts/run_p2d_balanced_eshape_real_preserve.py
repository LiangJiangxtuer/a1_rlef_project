...
def _balanced_loss(structure_preserve: float) -> dict:
    loss = _e_shape_loss(0.010)
    loss['structure_preserve'] = float(structure_preserve)
    loss['structure_preserve_grad'] = 0.5
    loss['structure_preserve_floor'] = 0.25
...
def planned_runs() -> list[dict]:
    return [
        {
            'id': 'P2D_ESHAPE010_STRUCT002',
            'name': 'eshape010_structure_preserve002',
            'seed': 3407,
            'base_variant': 'P2C_DGB_ESHAPE010',
            'intervention': 'generic_structure_preserve',
            'model': _e_shape_model(),
            'loss': _balanced_loss(0.02),
            'rationale': 'P2D balanced branch: keep e_shape=0.010 UEFB/synthetic direction and add weak generic contrast-weighted structure preservation, no dataset labels.',
        },
        {
            'id': 'P2D_ESHAPE010_STRUCT005',
            'name': 'eshape010_structure_preserve005',
            'seed': 3407,
            'base_variant': 'P2C_DGB_ESHAPE010',
            'intervention': 'generic_structure_preserve',
            'model': _e_shape_model(),
            'loss': _balanced_loss(0.05),
            'rationale': 'P2D balanced branch: stronger generic contrast-weighted structure preservation for real-domain recovery, no dataset labels.',
        },
    ]
...
def make_cfg(run: dict, max_steps: int = 1000, train_crop: int = 128, batch_size: int = 8) -> dict:
    return {
        ...
        'protocol': {
            'stage': 'P2D balanced e_shape010 real preservation',
            'variant_id': run['id'],
            'variant_name': run['name'],
            'base_variant': run['base_variant'],
            'intervention': run['intervention'],
            'notes': run['rationale'],
            'phase2d_balanced_eshape_real_preserve': True,
        },
        'model': run['model'],
        'training': {
...
def run_one(run: dict, max_steps: int, train_crop: int, batch_size: int, gpu: int, resume: bool = False) -> dict:
    tag = f"p2d_{run['id'].lower()}_{run['name']}"
    ...
    log_path = ROOT / 'logs/p2d_balanced_eshape_real_preserve' / f'{tag}.log'
    ...
    row = {
        ...
        'structure_preserve': float(run['loss'].get('structure_preserve', 0.0)),
...
def write_summary(rows: list[dict]) -> Path:
    ...
    table = ROOT / 'results/tables/p2d_balanced_eshape_real_preserve_summary.csv'
    table.parent.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for row in rows for k in row.keys()})
    preferred = ['run','variant_id','variant_name','base_variant','intervention','seed','backbone','backbone_blocks','gauge_mode','route_type','route_floor','e_shape','structure_preserve','safe_alpha','steps','train_crop','batch_size','gpu','elapsed_sec_train','elapsed_sec_total','uefb_psnr','uefb_ssim','uefb_E_MAE','uefb_E_MAE_aligned','uefb_E_corr','uefb_identity_drop','uefb_q_ece','real_psnr','real_ssim','real_lee','real_nai','real_E_corr','real_identity_drop','real_q_ece','synthetic_psnr','synthetic_ssim','synthetic_lee','synthetic_nai','synthetic_E_corr','synthetic_identity_drop','synthetic_q_ece','run_dir','config','log']
    ...
    (ROOT / 'results/tables/p2d_balanced_eshape_real_preserve_summary.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    return table
...
def main() -> None:
    ...
    runs = planned_runs()
    print(json.dumps({'P2D_START': True, 'runs': [(r['id'], r['name']) for r in runs], 'max_steps': args.max_steps, 'parallel': args.parallel, 'gpu_start': args.gpu_start}, indent=2), flush=True)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=max(1, args.parallel)) as ex:
        ...
        for fut in as_completed(futures):
            row = fut.result()
            rows.append(row)
            print(json.dumps({'DONE_RUN': row['run'], 'structure_preserve': row.get('structure_preserve'), 'uefb_psnr': row.get('uefb_psnr'), 'real_psnr': row.get('real_psnr'), 'synthetic_psnr': row.get('synthetic_psnr'), 'uefb_E_corr': row.get('uefb_E_corr')}, ensure_ascii=False), flush=True)
    table = write_summary(rows)
    print(json.dumps({'P2D_DONE': True, 'summary': str(table), 'rows': rows}, indent=2), flush=True)
