...
def _domain_model(adapter: str) -> dict:
    model = dict(P6_MODEL)
    model.update({
...
def planned_runs() -> list[dict]:
    specs = [
        ('P7_MS_DGATE', 'm4j_eshape_multiscale_domain_gate_bias', 'gate_bias', 'domain-conditioned gate bias only'),
        ('P7_MS_DHEAD', 'm4j_eshape_multiscale_domain_head_bias', 'head_bias', 'domain-conditioned restoration/exposure head biases'),
        ('P7_MS_DAFFINE_GATE', 'm4j_eshape_multiscale_domain_affine_gate', 'feature_affine+gate_bias', 'domain-conditioned feature affine adapter plus gate bias'),
    ]
    return [
        {
            'id': variant_id,
            'name': name,
            'seed': 3407,
            'model': _domain_model(adapter),
            'loss': _domain_loss(),
            'domain_adapter': adapter,
            'rationale': f'P7 multiscale trunk with {description}; no teacher/output distillation and no scalar dataset reconstruction upweighting.',
        }
        for variant_id, name, adapter, description in specs
    ]
...
def make_cfg(run: dict, max_steps: int = 1000, train_crop: int = 128, batch_size: int = 8) -> dict:
    return {
        ...
        'protocol': {
            'stage': 'P7 domain-conditioned head structural route',
            'variant_id': run['id'],
            'variant_name': run['name'],
            'domain_adapter': run['domain_adapter'],
            'notes': 'Keep P6 multiscale trunk but learn tiny domain-conditioned calibration adapters/heads instead of global scalar rec/gate knobs.',
        },
        'model': run['model'],
        'training': {
...
def write_summary(rows: list[dict]) -> Path:
    ...
    table = ROOT / 'results/tables/p7_domainhead_summary.csv'
    table.parent.mkdir(parents=True, exist_ok=True)
    fields = sorted({k for row in rows for k in row.keys()})
    preferred = ['run','variant_id','variant_name','seed','backbone','backbone_blocks','domain_adapter','steps','train_crop','batch_size','gpu','elapsed_sec_train','elapsed_sec_total','uefb_psnr','uefb_ssim','uefb_E_MAE','uefb_E_MAE_aligned','uefb_E_corr','real_psnr','real_ssim','real_E_corr','synthetic_psnr','synthetic_ssim','synthetic_E_corr','run_dir','config','log']
    ...
    (ROOT / 'results/tables/p7_domainhead_summary.json').write_text(json.dumps(payload, indent=2), encoding='utf-8')
    return table
...
def main() -> None:
    ...
    runs = planned_runs()
    print(json.dumps({'P7_START': True, 'runs': [(r['id'], r['name']) for r in runs], 'max_steps': args.max_steps, 'parallel': args.parallel, 'gpu_start': args.gpu_start}, indent=2), flush=True)
    rows: list[dict] = []
    with ThreadPoolExecutor(max_workers=max(1, args.parallel)) as ex:
        ...
        for fut in as_completed(futures):
            row = fut.result()
            rows.append(row)
            print(json.dumps({'DONE_RUN': row['run'], 'domain_adapter': row.get('domain_adapter'), 'uefb_psnr': row.get('uefb_psnr'), 'real_psnr': row.get('real_psnr'), 'synthetic_psnr': row.get('synthetic_psnr')}, ensure_ascii=False), flush=True)
    table = write_summary(rows)
    print(json.dumps({'P7_DONE': True, 'summary': str(table), 'rows': rows}, indent=2), flush=True)
