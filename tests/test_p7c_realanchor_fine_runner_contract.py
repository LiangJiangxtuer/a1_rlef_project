from scripts.run_p7c_realanchor_fine import make_cfg, planned_runs, should_promote_to_multiseed, gpu_for_run


def test_p7c_plans_stronger_dhead_real_anchor_fine_sweep():
    runs = planned_runs()
    ...
    assert [r['real_anchor_weight'] for r in runs] == [0.015, 0.020, 0.030]
    for run in runs:
...
def test_p7c_config_marks_fine_sweep_protocol_and_keeps_joint_data():
    cfg = make_cfg(planned_runs()[1], max_steps=17, train_crop=96, batch_size=5)
    assert cfg['protocol']['stage'] == 'P7c domain-head real-anchor fine route'
    assert cfg['protocol']['real_anchor_weight'] == 0.020
    assert cfg['model']['domain_adapter'] == 'head_bias'
    assert cfg['loss']['domain_head_anchor_by_dataset']['real'] == 0.020
...
def test_p7c_promotion_rule_requires_balanced_psnr_gain():
    p3c = {
        'uefb_psnr': {'mean': 17.9},
        'real_psnr': {'mean': 20.0},
...
def test_p7c_gpu_assignment_can_skip_busy_gpu_zero():
